 ...
 * Handles quick mood check-in CRUD operations
 */
...
// Contributing factor labels
export const CONTRIBUTING_FACTOR_LABELS: Record<ContributingFactor, { label: string; icon: string }> = {
    exercise: { label: 'Exercise', icon: 'run' },
    diet: { label: 'Diet', icon: 'food-apple' },
    sleep: { label: 'Sleep', icon: 'sleep' },
    mood: { label: 'Mood', icon: 'emoticon-happy' },
    work: { label: 'Work', icon: 'briefcase' },
    social: { label: 'Social', icon: 'account-group' },
    health: { label: 'Health', icon: 'heart-pulse' },
    weather: { label: 'Weather', icon: 'weather-sunny' },
    stress: { label: 'Stress', icon: 'alert-circle' },
    relaxation: { label: 'Relaxation', icon: 'meditation' }
};
